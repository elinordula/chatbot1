        ...
        for tag in matched_tags:
            ...
            for intent in intents['intents']:
                if intent['tag'] == tag:
                    ...
                    break
        # Join responses with two newlines for spacing
        combined_response = "\n\n".join(responses)
...
with chat_container:
    ...
    for chat in st.session_state["chat_history"]:
        if chat["sender"] == "user":
            st.markdown(f"<div style='text-align: right;'><b>User:</b> {chat['message']}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div style='text-align: left;'><b>Chatbot:</b> {chat['message']}</div>", unsafe_allow_html=True)

    # Scrollbar to allow the user to scroll through the chat history
    st.markdown(
        """
        <style>
        .element-container {
...
# Input box
user_input = st.text_input("Type your question here...", key="user_input", max_chars=100, label_visibility="collapsed")

# Button row: Send and Clear
...
if col1.button("Send"):
    ...
    if user_input.strip():
        ...
        st.session_state["chat_history"].append({"sender": "chatbot", "message": f"Chatbot's response to '{user_input}'"})  # Replace with chatbot logic
...
# ---------------------------
# Backend Chatbot Logic (simplified)
# ---------------------------

departments = {
    "cse": ["Dr. N. Bhalaji – Principal", "Dr. D. C. Joy Winnie Wise – Professor"],
    "ece": ["Dr. Sundar Rangarajan – Professor", "Dr. G. Nirmala Priya – Professor"]
}

def get_response(user_message):
    user_message = user_message.lower()

    # Check if the user asked for department members
    if "cse" in user_message:
        return "Here are the members of the CSE department: " + ", ".join(departments["cse"])
    elif "ece" in user_message:
        return "Here are the members of the ECE department: " + ", ".join(departments["ece"])
    else:
        return "I am sorry, I do not understand your question."

# Append chatbot responses to chat history based on user input
if user_input and st.session_state.get("chat_history", []):
    chatbot_response = get_response(user_input)
    st.session_state["chat_history"].append({"sender": "chatbot", "message": chatbot_response})
